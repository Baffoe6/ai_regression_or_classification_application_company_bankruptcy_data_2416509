...
def run_simple_pipeline():
    ...
    try:
        ...
        from src.models import LogisticRegressionModel, RandomForestModel, ModelEvaluator, ModelFactory
        ...
        X_train, X_test, y_train, y_test, exploration_stats = processor.process_full_pipeline()
        ...
        for model_config in DEFAULT_MODEL_CONFIGS['logistic_regression']:
            ...
            print(f"     ✅ {model_config.name} - F1: {result['f1_score']:.4f}, Accuracy: {result['accuracy']:.4f}")
        ...
        for result in sorted(results, key=lambda x: x['f1_score'], reverse=True):
            print(f"{result['model_name']:<20} | F1: {result['f1_score']:.4f} | Acc: {result['accuracy']:.4f} | AUC: {result['roc_auc']:.4f}")
        ...
        for model_name, model in models.items():
            if hasattr(model, 'get_feature_importance') and model.get_feature_importance() is not None:
                ...
                feature_names = processor.feature_names or [f"Feature_{i}" for i in range(len(importance))]
        ...
        print("\n💡 Next Steps:")
        print("   • Run the full optimized notebook: jupyter notebook bankruptcy_prediction_optimized.ipynb")
