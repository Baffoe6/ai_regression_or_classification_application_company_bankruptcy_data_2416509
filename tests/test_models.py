...
class TestDataProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data = {
            f'X{i+1}': np.random.randn(n_samples) for i in range(n_features)
        }
...
class TestBankruptcyPredictor(unittest.TestCase):
    ...
    def _create_mock_data(self):
        ...
        y = (X[:, 0] + X[:, 1] * 0.5 + np.random.randn(n_samples) * 0.5 < -0.5).astype(int)
    ...
    def test_data_loading_and_processing(self, mock_processor):
        ...
        mock_instance.process_full_pipeline.return_value = (
            self.predictor.X_train, self.predictor.X_test,
            self.predictor.y_train, self.predictor.y_test,
            self.predictor.exploration_stats
        )
