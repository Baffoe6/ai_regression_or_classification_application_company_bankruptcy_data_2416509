...
class TestDataProcessor:
    ...
    def sample_data(self):
        ...
        data = {
            'ROA(C)_before_interest_and_depreciation_before_interest': np.random.normal(0.1, 0.05, n_samples),
            'ROA(A)_before_interest_and_%_after_tax': np.random.normal(0.08, 0.04, n_samples),
            'ROA(B)_before_interest_and_depreciation_after_tax': np.random.normal(0.09, 0.045, n_samples),
    ...
    def test_memory_efficiency(self, processor):
        ...
        assert isinstance(X, pd.DataFrame)
        assert len(X) == 10000
        assert X.memory_usage(deep=True).sum() < large_data.memory_usage(deep=True).sum()
