    ...
    def validate(self, df: pd.DataFrame) -> Tuple[bool, List[str]]:
        ...
        for column in self.required_columns:
            ...
            if column not in self.nullable_columns and df[column].isnull().any():
                errors.append(
                    f"Column '{column}' contains null values but is not nullable"
                )
            ...
            # Check value ranges
            if column in self.value_ranges and pd.api.types.is_numeric_dtype(
                df[column]
            ):
            ...
            if column in self.categorical_values:
                invalid_values = set(df[column].dropna().unique()) - set(
                    self.categorical_values[column]
                )
                if invalid_values:
                    errors.append(
...
class DataQualityAnalyzer:
    ...
    def analyze(self, df: pd.DataFrame, target_column: str = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"Data quality analysis completed. Overall score: {overall_score:.2f}"
        )
    ...
    def _check_completeness(
        ...
        if high_missing_columns:
            ...
            recommendations.append(
                "Consider removing or imputing high missing value columns"
            )
    ...
    def _check_uniqueness(
        ...
        for column in df.columns:
            ...
            if unique_ratio < 0.01 and column != target_column:
                issues.append(
                    f"Column '{column}' has very low uniqueness ({unique_ratio:.1%})"
                )
                recommendations.append(
                    f"Consider removing low-variance column '{column}'"
                )
    ...
    def _check_validity(
        ...
        for column in potential_ratio_columns:
            if column in numeric_columns:
                ...
                if negative_count > len(df) * 0.1:  # More than 10% negative
                    validity_issues += 1
                    issues.append(
                        f"Column '{column}' has many negative values ({negative_count})"
                    )
                    recommendations.append(
                        f"Investigate negative values in ratio column '{column}'"
                    )
        ...
        if target_column and target_column in df.columns:
            ...
            if len(unique_targets) < 2:
                validity_issues += 1
                issues.append(
                    f"Target variable has only {len(unique_targets)} unique values"
                )
                ...
                recommendations.append(
                    "Ensure target variable is properly encoded as 0/1"
                )
    ...
    def _check_consistency(
        ...
        for column in df.columns:
            ...
            if df[column].dtype == "object":
                ...
                try:
                    ...
                    issues.append(
                        f"Column '{column}' appears to be numeric but stored as object"
                    )
                    recommendations.append(
                        f"Convert '{column}' to appropriate numeric type"
                    )
        ...
        for column in numeric_columns:
            if column != target_column:
                ...
                IQR = Q3 - Q1
                outliers = (
                    (df[column] < Q1 - 3 * IQR) | (df[column] > Q3 + 3 * IQR)
                ).sum()
        ...
        consistency_score = (
            1 - (consistency_issues / total_checks) if total_checks > 0 else 1
        )
    ...
    def _check_accuracy(
        ...
        for column in df.columns:
            ...
            for check_type, (min_val, max_val) in financial_checks.items():
                if check_type in column_lower and pd.api.types.is_numeric_dtype(
                    df[column]
                ):
                    total_checks += 1
                    out_of_range = (
        ...
        if target_column and target_column in df.columns:
            ...
            if len(class_counts) >= 2:
                ...
                if minority_ratio < 0.01:  # Less than 1% minority class
                    accuracy_issues += 1
                    issues.append(
                        f"Severe class imbalance: minority class is {minority_ratio:.1%}"
                    )
...
def analyze_data_quality(
    df: pd.DataFrame, target_column: str = "Bankrupt?"
) -> Dict[str, Any]:
    """
