...
class DataProcessor:
    ...
    def explore_data(self, df: pd.DataFrame) -> dict:
        ...
        exploration_stats = {
            ...
            'target_distribution': df[self.config.target_column].value_counts().to_dict(),
            'feature_count': len([col for col in df.columns if col != self.config.target_column]),
    ...
    def preprocess_data(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.config.feature_selection and self.config.max_features:
            logger.info(f"Performing feature selection to top {self.config.max_features} features")
            self.feature_selector = SelectKBest(score_func=f_classif, k=self.config.max_features)
    ...
    def process_full_pipeline(self, data_path: Optional[str] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, dict]:
