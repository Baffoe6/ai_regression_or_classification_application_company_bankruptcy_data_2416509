...
class DataProcessor:
    ...
    def validate_data(self, df: pd.DataFrame) -> Tuple[bool, str]:
        ...
        if self.config.target_column not in df.columns:
            return False, f"Target column '{self.config.target_column}' not found in data"
        
        # Check for minimum feature count
        feature_columns = [col for col in df.columns if col != self.config.target_column]
        if len(feature_columns) < 2:
            return False, f"Insufficient features: {len(feature_columns)} (minimum 2 required)"
        
        # Check target variable
        target_unique = df[self.config.target_column].nunique()
        if target_unique < 2:
            return False, f"Target variable has only {target_unique} unique values (minimum 2 required)"
        
        # Check for excessive missing values
        missing_percentage = df.isnull().sum().sum() / (len(df) * len(df.columns))
        if missing_percentage > 0.5:
            return False, f"Excessive missing values: {missing_percentage:.1%} (maximum 50% allowed)"
    ...
    def preprocess_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        logger.info(f"Preprocessing completed: {len(X)} samples, {len(X.columns)} features")
        return X, y
    ...
    def select_features(self, X: pd.DataFrame, y: pd.Series) -> pd.DataFrame:
        ...
        if len(X.columns) <= self.config.max_features:
            logger.info(f"Number of features ({len(X.columns)}) already <= max_features ({self.config.max_features})")
    ...
    def split_data(self, X: pd.DataFrame, y: pd.Series) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
        ...
        if y.nunique() >= 2 and y.value_counts().min() >= 2:
            ...
            stratify = None
            logger.warning("Stratification disabled due to insufficient samples in some classes")
