...
class BankruptcyPredictor:
    ...
    def load_and_process_data(self, data_path: str = None) -> None:
        ...
        self.logger.info(
            f"Data processing completed. Train shape: {self.X_train.shape}, Test shape: {self.X_test.shape}"
        )
    ...
    def visualize_data_exploration(self) -> None:
        ...
        if self.exploration_stats is None:
            self.logger.warning(
                "No exploration stats available. Run load_and_process_data first."
            )
            return
                ...
                try:
                    ...
                    if self.config.save_models:
                        model_path = os.path.join(
                            self.config.output_dir, "models", f"{config.name}.joblib"
                        )
    ...
    def analyze_feature_importance(self) -> None:
        ...
        for model_name, model in self.models.items():
            try:
                ...
                self.logger.error(
                    f"Failed to analyze feature importance for {model_name}: {str(e)}"
                )
    ...
    def get_best_model(self, metric: str = "f1_score") -> Tuple[str, Dict[str, Any]]:
        ...
        if not self.results:
            raise ValueError(
                "No evaluation results available. Call evaluate_models() first."
            )
        ...
        self.logger.info(
            f"Best model by {metric}: {best_model_name} ({best_result[metric]:.4f})"
        )
    ...
    def save_results(self, filepath: str = None) -> None:
        ...
        filepath = filepath or os.path.join(
            self.config.output_dir, "reports", "evaluation_results.json"
        )
    ...
    def generate_report(self) -> str:
        ...
        if self.exploration_stats:
            ...
            report.append(
                f"  Missing values: {self.exploration_stats['missing_values']}"
            )
            report.append(
                f"  Target distribution: {self.exploration_stats['target_distribution']}"
            )
            report.append("")
        ...
        sorted_results = sorted(
            self.results, key=lambda x: x.get("f1_score", 0), reverse=True
        )
        ...
        for result in sorted_results:
            ...
            if "cv_mean_accuracy" in result:
                report.append(
                    f"  CV Accuracy: {result['cv_mean_accuracy']:.4f} ± {result['cv_std_accuracy']:.4f}"
                )
        ...
        report_path = os.path.join(
            self.config.output_dir, "reports", "evaluation_report.txt"
        )
