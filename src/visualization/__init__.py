...
class BankruptcyVisualizer:
    ...
    def plot_target_distribution(self, df: pd.DataFrame, target_column: str = 'Bankrupt?', 
        ...
        fig = go.Figure(data=[
            go.Bar(
                ...
                text=[f'{count}<br>({percentage:.1f}%)' 
                      for count, percentage in zip(target_counts.values, target_percentages.values)],
                textposition='auto',
    ...
    def plot_feature_correlation_heatmap(self, df: pd.DataFrame, features: Optional[List[str]] = None,
                                       save_path: Optional[str] = None) -> go.Figure:
        """
    ...
    def plot_confusion_matrix(self, y_true: np.ndarray, y_pred: np.ndarray,
                            class_names: List[str] = None, save_path: Optional[str] = None) -> go.Figure:
        """
...
class Visualizer(BankruptcyVisualizer):
    ...
    def __init__(self, config: Config):
        ...
        logger.warning("Visualizer class is deprecated. Use BankruptcyVisualizer instead.")


# Export main classes and functions
__all__ = [
    'BankruptcyVisualizer',
    'Visualizer'  # Legacy support
]