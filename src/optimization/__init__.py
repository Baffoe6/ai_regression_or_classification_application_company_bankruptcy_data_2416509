...
class HyperparameterOptimizer:
    ...
    def optimize_hyperparameters(self, model_type: str, X_train: np.ndarray, y_train: np.ndarray,
                                cv_folds: int = 5, n_iter: int = 50, scoring: str = 'f1') -> Dict[str, Any]:
...
class FeatureSelector:
    ...
    def recursive_feature_elimination(self, estimator, X_train: np.ndarray, y_train: np.ndarray,
                                    n_features: int = 50, cv_folds: int = 5) -> np.ndarray:
    ...
    def model_based_selection(self, estimator, X_train: np.ndarray, y_train: np.ndarray,
                            threshold: str = 'median') -> np.ndarray:
        """Select features based on importance weights."""
        logger.info(f"Performing model-based feature selection with threshold={threshold}")
...
class EnsembleBuilder:
    ...
    def create_voting_ensemble(self, models: List[Tuple[str, BaseModel]], voting: str = 'soft') -> VotingClassifier:
    ...
    def create_bagging_ensemble(self, base_estimator, n_estimators: int = 10,
                              max_samples: float = 1.0, max_features: float = 1.0) -> BaggingClassifier:
    ...
    def create_stacked_ensemble(self, base_models: List[BaseModel], meta_model=None):
        ...
        return self.create_voting_ensemble([(f"model_{i}", model) for i, model in enumerate(base_models)])
...
class ModelOptimizer:
    ...
    def optimize_single_model(self, model_type: str, X_train: np.ndarray, y_train: np.ndarray,
                            X_test: np.ndarray, optimize_features: bool = True,
        ...
        if optimize_features and self.config.feature_selection:
            ...
            if model_type == 'random_forest':
                ...
                X_train_opt = self.feature_selector.model_based_selection(base_model, X_train_opt, y_train)
                X_test_opt = X_test_opt[:, self.feature_selector.selected_features_]
            else:
                # Use RFE for other models
                rf_selector = RandomForestClassifier(n_estimators=100, random_state=self.config.random_state)
                X_train_opt = self.feature_selector.recursive_feature_elimination(
                    rf_selector, X_train_opt, y_train, n_features=min(50, X_train_opt.shape[1]//2)
                )
                X_test_opt = X_test_opt[:, self.feature_selector.selected_features_]
            
            results['selected_features'] = self.feature_selector.get_selected_feature_indices()
        ...
        if optimize_hyperparams:
            ...
            # Use default model
            results['optimized_model'] = self.hyperparameter_optimizer._get_base_model(model_type)
    ...
    def create_optimized_ensemble(self, optimized_models: List[Dict[str, Any]], 
        ...
        voting_ensemble = self.ensemble_builder.create_voting_ensemble(ensemble_models, voting='soft')
        
        # Train ensemble
        voting_ensemble.fit(X_train, y_train)
        
        # Create bagging ensemble with best model
        best_model = max(optimized_models, key=lambda x: x.get('best_cv_score', 0))['optimized_model']
        bagging_ensemble = self.ensemble_builder.create_bagging_ensemble(best_model, n_estimators=10)
