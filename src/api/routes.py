...
def get_model_manager() -> ModelManager:
    ...
    if model_manager is None:
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Model manager initialization error"
            )
...
async def get_models():
    ...
    for name, model in loaded_models.items():
        ...
        if hasattr(model, "feature_importances_"):
            ...
            feature_importance = [
                FeatureImportance(
                    feature_name=f"X{i+1}", importance_score=float(score), rank=i + 1
                )
...
async def predict_batch(request: BatchPredictionRequest):
    ...
    bankruptcy_rate = (
        (bankruptcy_count / total_companies * 100) if total_companies > 0 else 0
    )
    ...
    try:
        ...
        return DataUploadResponse(
            message=(
                ...
                else "Data uploaded with validation warnings"
            ),
...
async def analyze_data_quality_endpoint(file_path: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Data quality analysis failed: {str(e)}"
        )
...
async def train_model(request: ModelTrainingRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        model_name = (
            f"{request.model_type.lower()}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
